...
The Store Code design resolves an existing shared Store folder. V0.9.4.74 requests `https://www.googleapis.com/auth/drive` for the controlled team test so a user can maintain their own Listings/More/Messaging workspace and read official Inventory. Full Drive access remains a production release gate: complete Google's required verification/security work or replace Store discovery with a narrower authenticated broker/picker flow before public launch.

OAuth **Test users** controls who can authorize LotKeys. Google Drive sharing separately controls Store access. In V0.9.4.74, Administrators are Store writers/managers; ordinary and Trusted users are Store/Inventory readers plus writers only on their own `Users/<name>` workspace. Do not share official Inventory as Editor with Trusted users—the V0.9.4.64 Store Processor applies their allowed automatic information changes and provides deduplicated Chat recovery delivery as the Admin Level 2 installer.

Admin Level 2 can use **Garage → Approved Users → Open Google OAuth Test Users ↗**, or open `https://console.cloud.google.com/auth/audience?project=lotkeys` directly. The Console link is not a credential: Google still requires sign-in and appropriate IAM access to the `lotkeys` Cloud project. Never publish or share an OAuth client secret or API key; the browser OAuth Client ID itself is public application configuration.

If a tester sees **Error 403: access_denied** and “has not completed the Google verification process,” add the exact email shown on that Google error page to **Audience → Test users**, wait briefly, and retry LotKeys authorization. Adding a person only to LotKeys Approved Users does not satisfy this separate Google testing gate.
...
- V0.9.4.74 requests `openid`, `email`, and `https://www.googleapis.com/auth/drive` for the controlled team test.
